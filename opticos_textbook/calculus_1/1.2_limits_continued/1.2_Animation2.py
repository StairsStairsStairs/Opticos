...
class hello(Scene):
    def construct(self):
        ...
        limitText = MathTex(r"\lim \limits_{x \to 0^-} f(x) = 3").to_edge(UL).shift([0, -2, 0])  
        ...
        self.remove(x_value)
        self.remove(y_value)
        x_value = DecimalNumber(num_decimal_places = 5).to_edge(UL).shift([1, 0, 0]).set_value(-0.99999)
        y_value = DecimalNumber(num_decimal_places = 5).to_edge(UL).shift([1.8, -1, 0]).set_value(3.00001)
        self.add(x_value)
        self.add(y_value)
