...
class Animation3(Scene):
    def construct(self):
        ...
        limitTextLeft = MathTex(r"\lim \limits_{x \to 0^-} f(x) = 1").to_edge(UL).shift([0, -2, 0])

        self.add(ax, labels, graph, hole, point, dot_left, x_value_left, xTextLeft, y_value_left, functionTextLeft)
        
        self.play(xTrackerLeft.animate.set_value(-1.05), run_time = 3)
        ...
        limitTextRight = MathTex(r"\lim \limits_{x \to 0^+} f(x) = 3").to_edge(UR).shift([0, -2, 0])

        self.add(dot_right, x_value_right, xTextRight, y_value_right, functionTextRight)
        
        self.play(xTrackerRight.animate.set_value(-0.95), run_time = 3)
        ...
        limitTextCenter = MathTex(r"\lim \limits_{x \to 0} f(x)\,\text{DNE}").shift([0, -2, 0])
