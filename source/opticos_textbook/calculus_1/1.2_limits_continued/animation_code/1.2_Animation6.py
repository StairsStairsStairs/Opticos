...
class Animation3(Scene):
    def construct(self):
        ...
        graph = ax.plot(func, color = functionColor, discontinuities=[-0.1, 0.1])
        ...
        limitTextLeft = MathTex(r"\lim \limits_{x \to 0^-} f(x) = \infty").to_edge(UL).shift([0, -2, 0])

        self.add(ax, labels, graph, dot_left, x_value_left, xTextLeft, y_value_left, functionTextLeft)
        
        self.play(xTrackerLeft.animate.set_value(-0.1), run_time = 3)
        ...
        y_value_left = DecimalNumber(num_decimal_places = 5).to_edge(UL).shift([1.8, -1, 0]).set_value(10000)
        ...
        dot_right = Dot(point=initial_point_right, color = 'red')
        ...
        limitTextRight = MathTex(r"\lim \limits_{x \to 0^+} f(x) = \infty").to_edge(UR).shift([0, -2, 0])

        self.add(dot_right, x_value_right, xTextRight, y_value_right, functionTextRight)
        
        self.play(xTrackerRight.animate.set_value(0.1), run_time = 3)
        ...
        y_value_right = DecimalNumber(num_decimal_places = 5).to_edge(UR).shift([-0.5, -1, 0]).set_value(10000)
        ...
        limitTextCenter = MathTex(r"\lim \limits_{x \to 0} f(x) = \infty").shift([0, -2, 0])
